...
function showToast(msg, tipo) {
  ...
  if (toastEl && tipo && colors[tipo]) {
    toastEl.style.borderLeft = "4px solid " + colors[tipo];
  } else if (toastEl) {
    toastEl.style.borderLeft = "";
  }
...
function nuevoSimple(tipo) {
  ...
  const validMsg =
    tipo === "Ocupación"
      ? "Ingrese el nombre de la ocupación"
      : tipo === "Grupo Ocupacional"
        ? "Ingrese el nombre del grupo ocupacional"
        : "⚠️ Por favor, complete los campos obligatorios";
  const screenId =
    tipo === "Ocupación"
      ? "screen-ocupaciones"
      : tipo === "Grupo Ocupacional"
  ...
  if (btnSave) {
    btnSave.onclick = () => {
      ...
      const cod = document.getElementById("sCod").value.trim();
      if (!desc) {
        showToast(validMsg, "error");
        document.getElementById("sDesc").focus();
        return;
      }
      // Lógica de confirmación según tipo
      const msgConfirm =
        tipo === "Ocupación"
          ? '"' + desc + '" en Ocupaciones'
          : "Grupos Ocupacionales";

      confirmarGuardar(msgConfirm, () => {
        // Cerrar modal
        bootstrap.Modal.getInstance(
          document.getElementById("modalSimple"),
        ).hide();
        // Inyectar tarjeta real en la lista
        if (screenId) {
          const nuevoId =
            cod ||
            (tipo === "Ocupación" ? "OC" : "GO") +
              "-" +
              String(Date.now()).slice(-3);
          const screen = document.getElementById(screenId);
          const firstCard = screen?.querySelector(".card-item");
          const container = firstCard?.parentElement || screen;
          if (container) {
            const card = document.createElement("div");
            card.className = "card-item";
            card.innerHTML = `
              <div class="flex-grow-1 d-flex align-items-center gap-3">
                <strong style="color: var(--primary)">${nuevoId}</strong>
                <span class="muted-dot">•</span>
                <span class="fw-semibold">${desc}</span>
                <span class="b-activo ms-auto">Activo</span>
              </div>
              <div class="d-flex gap-2">
                <button class="btn btn-light rounded-circle p-0" style="width:32px;height:32px"
                  title="${tipo === "Grupo Ocupacional" ? "Editar Grupo" : "Editar " + tipo}"
                  onclick="editarSimple('${nuevoId}','${desc}','${tipo}')"
                  data-bs-toggle="modal" data-bs-target="#modalSimple">
                  <i class="fas fa-edit text-muted"></i>
                </button>
                <button class="btn btn-light rounded-circle text-danger p-0" style="width:32px;height:32px"
                  title="${tipo === "Grupo Ocupacional" ? "Eliminar Grupo" : "Eliminar " + tipo}"
                  onclick="eliminarRegistro(this,'card-item')">
                  <i class="fas fa-trash"></i>
                </button>
              </div>`;
            // Insertar antes de la paginación
            const nav = screen.querySelector("nav[aria-label='Paginación']");
            nav
        ...
        const successMsg =
          tipo === "Grupo Ocupacional"
            ? "Grupo Ocupacional actualizado correctamente"
            : "✅ " + desc + " guardado en " + moduloName + ".";
...
function editarSimple(cod, desc, tipo = "Registro") {
  ...
  if (btnSave) {
    btnSave.onclick = () => {
      const descVal = document.getElementById("sDesc").value.trim();
      if (!descVal) {
        showToast(validMsg, "error");
        document.getElementById("sDesc").focus();
        return;
      ...
      const msgConfirm =
        tipo === "Ocupación"
          ? '"' + descVal + '" en Ocupaciones'
          : "Grupos Ocupacionales";

      confirmarGuardar(msgConfirm, () => {
        // Actualizar el texto de la tarjeta en la lista
        const allCards = document.querySelectorAll(".card-item");
        allCards.forEach((card) => {
          const strongEl = card.querySelector("strong");
          if (strongEl && strongEl.textContent.trim() === cod) {
            const spanDesc = card.querySelector(".fw-semibold");
            if (spanDesc) spanDesc.textContent = descVal;
        ...
        const successMsg =
          tipo === "Grupo Ocupacional"
            ? "Grupo Ocupacional actualizado correctamente"
            : "✅ " + descVal + " actualizado en " + moduloName + ".";
        showToast(successMsg, "success");
      });
    };
  }
...
function editarPrestacion(cod) {
  ...
  if (cod === "PR-PSI-001") {
    ...
    document.getElementById("pDesc").value = "Psicología Ocupacional – Altura";
    document.getElementById("pEstado").value = "cesado";
    document.getElementById("pTipoEx").value = "altura";
    icon = "fa-helmet-safety";
    resetBox("boxPF", "emptyPF", "fas fa-inbox", "Sin fichas vinculadas.");
    pushItem("boxPF", "emptyPF", rowPF("CPSG-001", "Competencias Laborales"));
  }
...
function editarFicha(cod, esGrupo) {
  ...
  if (!esGrupo) {
    ...
    pushItem(
      "boxCD",
      "emptyCD",
      rowCD("INT-001", "Interpretación Clínica", "interpretacion", "activo"),
    );
  } else {
...
function addCD() {
  ...
  ["cdCod", "cdDes"].forEach((id) => (document.getElementById(id).value = ""));
...
function addSF() {
  const cod = document.getElementById("sfCod").value.trim().toUpperCase(),
    des =
      document.getElementById("sfDes").value.trim() || FICHAS_DB[cod] || "—";
  if (!cod) {
                     ...
                     <table class="table table-sm table-bordered text-center align-middle mb-0" style="font-size: 0.85rem;">
                        ...
                        <tbody>
                           <tr>
                              ...
                              <td><input type="number" class="form-control form-control-sm mx-auto text-success fw-bold text-center border-secondary" style="width: 50px;" value="1" step="0.5"></td>
                              <td><input type="number" class="form-control form-control-sm mx-auto text-success fw-bold text-center border-secondary" style="width: 50px;" value="2" step="0.5"></td>
                              <td><input type="number" class="form-control form-control-sm mx-auto text-success fw-bold text-center border-secondary" style="width: 50px;" value="3" step="0.5"></td>
                           </tr>
                           <tr>
    ...
    if (d.step >= 2) {
      if (cod === "PSI-ANS-001") {
        pushItem(
          ...
          rowPregunta(
            1,
            "¿Con qué frecuencia siente tensión muscular?",
            "cerrada",
          ),
        );
        pushItem(
          "boxPreg",
          "emptyPreg",
          rowPregunta(2, "¿Dibuja a una persona bajo la lluvia?", "abierta"),
        );
        pushItem(
          "boxPreg",
          "emptyPreg",
          rowPregunta(
            3,
            "Indique los problemas que reconoce frecuentemente",
            "cerrada",
          ),
        );
        ...
        pushItem(
          ...
          rowPregunta(
            1,
            "¿Cuál fue el motivo de salida de su anterior empleo?",
            "abierta",
          ),
        );
        pushItem(
          "boxPreg",
          "emptyPreg",
          rowPregunta(2, "Describa tres fortalezas y debilidades", "abierta"),
        );
        pushItem(
          "boxPreg",
          "emptyPreg",
          rowPregunta(3, "¿Cómo reacciona ante la presión?", "abierta"),
        );
      } else {
        pushItem(
          "boxPreg",
          ...
          rowPregunta(
            1,
            "Pregunta de prueba generada automáticamente",
            "cerrada",
          ),
        );
      }
    ...
    if (d.step >= 3) {
      if (cod === "PSI-ANS-001") {
        ...
        pushItem(
          "boxFac",
          "emptyFac",
          rowFactor("FAC-ENT-A", "Competencias Blandas"),
        );
        pushItem(
          "boxFac",
          "emptyFac",
          rowFactor("FAC-ENT-B", "Estabilidad Emocional"),
        );
      }
    }
    if (d.step >= 5) {
    ...
    html = `
            <div class="p-3 mb-0 border rounded shadow-sm bg-white border-top border-4" style="border-top-color: #f59e0b !important;">
              ...
              <div class="row g-3">
                  ...
                  <div class="col-md-4 mt-2" id="mp_A_boxVineta">
                    <label class="form-label text-muted small fw-bold mb-1">Tipo de Viñeta</label>
                    <select class="form-select form-select-sm" id="mp_vineta" onchange="document.getElementById('mp_A_vineta_otros_box').style.display = this.value === 'otros' ? 'block' : 'none'">
                    ...
                    <div id="mp_A_vineta_otros_box" style="display:none;" class="mt-2">
                       <input type="text" class="form-control form-control-sm" id="mp_A_vineta_otros_input" placeholder="Especificar viñeta">
                    </div>
                  </div>
            ...
            <div class="p-3 mb-0 border rounded shadow-sm bg-white border-top border-4" style="border-top-color: #3b82f6 !important;">
              ...
              <div class="row g-3">
                  ...
                  <div class="col-md-4 mt-2">
                    <label class="form-label text-muted small fw-bold mb-1">Tipo de Viñeta</label>
                    <select class="form-select form-select-sm" id="mp_vineta" onchange="document.getElementById('mp_vineta_otros_box').style.display = this.value === 'otros' ? 'block' : 'none'">
                      <option value="num">Números (1, 2, 3)</option>
                      <option value="min">Letras Minúsculas (a, b, c)</option>
                      <option value="may" selected>Letras Mayúsculas (A, B, C)</option>
                      <option value="otros">Otros</option>
                    </select>
                    <div id="mp_vineta_otros_box" style="display:none;" class="mt-2">
                      <input type="text" class="form-control form-control-sm" id="mp_vineta_otros_input" placeholder="Especificar viñeta">
                    </div>
                  </div>
    ...
    html = `
            <div class="p-3 mb-0 border rounded shadow-sm bg-white border-top border-4" style="border-top-color: #10b981 !important;">
              ...
              <div class="row g-3">
                  <div class="col-md-12">
                    <label class="form-label fw-bold small text-primary mb-1">Descripción de Pregunta</label>
                    <input type="text" class="form-control" id="mp_N_text" placeholder="Escribe tu pregunta numérica aquí...">
                  </div>
                  <div class="col-md-6">
            ...
            <div class="p-3 mb-0 border rounded shadow-sm bg-white border-top border-4" style="border-top-color: #8b5cf6 !important;">
              ...
              <div class="row g-3">
                 <div class="col-md-12">
                   ...
                   <input type="text" class="form-control" placeholder="Escribe la pregunta matriz...">
                 </div>
                 <div class="col-md-6">
                   <label class="form-label text-muted small fw-bold mb-1">Filas (Sub-preguntas)</label>
                   <textarea class="form-control" rows="3" placeholder="Fila 1\\nFila 2\\n..."></textarea>
                 </div>
                 <div class="col-md-6">
                   <label class="form-label text-muted small fw-bold mb-1">Columnas (Opciones)</label>
                   <textarea class="form-control" rows="3" placeholder="Opción 1\\nOpción 2\\n..."></textarea>
                 </div>
              </div>
            </div>`;
...
function ctrlAbierta() {
  ...
  const countFields = isMulti
    ...
    : 1;
  const boxVinetaA = document.getElementById("mp_A_boxVineta");
  if (boxVinetaA) boxVinetaA.style.display = isMulti ? "none" : "block";
  ...
  // Viñetas: Quitar selector cuando el tipo es pregunta abierta con múltiples respuestas
  const boxVineta = document.getElementById("mp_A_boxVineta");
  if (boxVineta) boxVineta.style.display = isMulti ? "none" : "block";
}
...
function addMpcAlternativa() {
  ...
  div.innerHTML = `
            ...
            <input type="number" class="pos-input" value="${n}" min="1" style="width: 40px;" onchange="reordenarPorInput(this, '#mp_C_alternativas_box')">
            ...
            <div style="width: 70px;" class="mpc-pts-box"><input type="number" class="form-control form-control-sm border-primary text-center" placeholder="Pts" value="0" min="0" step="0.1"></div>
...
function addMpaAlternativa() {
  ...
  div.innerHTML = `
            <i class="fas fa-grip-vertical text-black-50 handle" style="cursor: grab;"></i>
            <input type="number" class="pos-input" value="${n}" min="1" style="width: 40px;" onchange="reordenarPorInput(this, '#mp_A_alternativas_box')">
            <div style="width: 60px;">
            ...
            <div style="width: 70px;" class="mpc-pts-box"><input type="number" class="form-control form-control-sm border-warning text-center" placeholder="Pts" value="0" min="0" step="0.1"></div>
            <button class="btn btn-sm btn-outline-primary border-0" title="Duplicar" onclick="duplicarAlternativa(this)"><i class="fas fa-copy"></i></button>
            <button class="btn btn-sm btn-outline-danger border-0" title="Eliminar" onclick="const p = this.closest('[id]'); this.closest('.item-alt').remove(); renumerarContenedor('#' + p.id);"><i class="fas fa-times"></i></button>
         `;
  box.appendChild(div);
...
function previewImgAlt(input) {
  const preview = input.parentElement.querySelector(".img-preview-circle");
...
function rowFactor(cod, desc, formula = "") {
  const fBadge = formula
    ...
    : "";
  return `<div class="item-row"><div class="inf"><span class="tag tag-punt fw-bold">${cod}</span><span class="fw-medium">${desc}</span>${fBadge}</div><button class="btn btn-light rounded-circle text-danger p-0" style="width:32px;height:32px;" onclick="this.closest('.item-row').remove()"><i class="fas fa-times"></i></button></div>`;
}
